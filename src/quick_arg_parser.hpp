...
template <typename T, typename SFINAE = void> struct ArgConverter {
...
struct ArgConverter<T,
  ...
  static T makeDefault() { return 0; }
  static T deserialise(const std::string &from) { return std::stoi(from); }
...
struct ArgConverter<
  ...
  static T makeDefault() { return 0; }
  static T deserialise(const std::string &from) { return std::stof(from); }
  constexpr static bool canDo = true;
};

template <> struct ArgConverter<std::string, void> {
  static std::string makeDefault() { return ""; }
  static std::string deserialise(const std::string &from) { return from; }
  constexpr static bool canDo = true;
};

template <typename T> struct ArgConverter<std::shared_ptr<T>, void> {
  static std::shared_ptr<T> makeDefault() { return nullptr; }
...
template <typename T> struct ArgConverter<std::unique_ptr<T>, void> {
  static std::unique_ptr<T> makeDefault() { return nullptr; }
...
struct ArgConverter<std::vector<T>,
                    typename std::enable_if<ArgConverter<T>::canDo>::type> {
  static std::vector<T> makeDefault() { return {}; }
...
template <typename T>
struct ArgConverter<std::unordered_map<std::string, T>,
                    typename std::enable_if<ArgConverter<T>::canDo>::type> {
  static std::unordered_map<std::string, T> makeDefault() { return {}; }
  static std::unordered_map<std::string, T>
  deserialise(const std::vector<std::string> &from) {
      ...
      for (int i = 0; i < int(part.size()) + 1; i++) {
        if (part[i] == ',' || i == int(part.size())) {
          ...
          if (separator == std::string::npos)
            throw ArgumentError("Argument is expected to be a comma separated "
                                "list of name-value pairs separated by '='");
          made[section.substr(0, separator)] =
              ArgConverter<T>::deserialise(section.substr(separator + 1));
          lastPosition = i + 1;
        }
      }
    }
...
#if __cplusplus > 201402L
  void operator=(std::nullopt_t) { operator=(nullptr); }
#endif
  T &operator*() { return *reinterpret_cast<T *>(_contents.data()); }
...
template <typename T> struct ArgConverter<Optional<T>, void> {
  static Optional<T> makeDefault() { return nullptr; }
...
#if __cplusplus > 201402L
template <> struct ArgConverter<std::filesystem::path, void> {
  static std::filesystem::path makeDefault() { return {}; }
...
template <typename T, typename SFINAE = void> struct Demultiplexer {
...
template <typename T>
struct Demultiplexer<T, typename std::enable_if<
                            !std::is_void<decltype(ArgConverter<T>::deserialise(
                                std::declval<std::string>()))>::value>::type> {
  static T deserialise(const std::vector<std::string> &multiplexed) {
    if (multiplexed.size() > 1)
      throw ArgumentError(
          "Argument was not expected to appear more than once (" +
          multiplexed[1] + " is excessive)");
    return ArgConverter<T>::deserialise(multiplexed[0]);
  }
};

template <typename T, typename SFINAE = void> struct HelpProvider {
  template <typename F>
...
template <typename T>
struct HelpProvider<T, typename std::enable_if<!std::is_void<decltype(T::help(
                           std::declval<std::string>()))>::value>::type> {
  template <typename F>
...
template <typename T, typename SFINAE = void> struct OnHelpCallback {
  template <typename F> static void on(T *, const F &ifAbsent) { ifAbsent(); }
};

template <typename T>
struct OnHelpCallback<T, typename std::enable_if<std::is_void<decltype(
                             std::declval<T>().onHelp())>::value>::type> {
  template <typename F> static void on(T *instance, const F &) {
    instance->onHelp();
  }
};
...
template <typename T>
struct HasHelpOptionsProvider<
    T,
    typename std::enable_if<!std::is_void<decltype(T::options())>::value>::type>
    : std::true_type {};

template <typename T, typename SFINAE = void> struct VersionPrinter {
  static bool print() { return false; }
};

template <typename T>
struct VersionPrinter<T, typename std::enable_if<!std::is_void<decltype(
                             std::string(T::version))>::value>::type> {
  static bool print() {
...
template <typename T, typename SFINAE = void> struct OnVersionCallback {
  template <typename F> static void on(T *, const F &ifAbsent) { ifAbsent(); }
};

template <typename T>
struct OnVersionCallback<T, typename std::enable_if<std::is_void<decltype(
                                std::declval<T>().onVersion())>::value>::type> {
  template <typename F> static void on(T *instance, const F &) {
    instance->onVersion();
  }
};
...
// MSVC likes converting const char* literals to initialiser lists and causing
// ambiguous calls with it
...
struct StringFilterOk<
    T, typename std::enable_if<
           (std::is_class<T>::value && !IsInitializerList<T>::value) ||
           std::is_arithmetic<T>::value || std::is_floating_point<T>::value ||
           std::is_enum<T>::value>::type> : std::true_type {};
#endif

struct DummyValidator {};

template <typename Validator, typename SFINAE = void> struct ValidatorUser {
...
struct ValidatorUser<Validator, typename std::enable_if<!std::is_same<
                                    Validator, DummyValidator>::value>::type> {
  template <typename Value,
            typename std::enable_if<std::is_same<
                bool, decltype(std::declval<Validator>()(
                          std::declval<Value>()))>::value>::type * = nullptr>
  static bool useValidator(const Validator &validator, const Value &value) {
...
template <typename Child> class MainArguments {
  std::string _programName;
  std::vector<std::string> _argv;

  enum InitialisationStep { UNINITIALISED, INITIALISING, INITIALISED };
  struct Singleton {
    ...
    std::vector<std::string>
        confusingSwitches; // nonstandard switches starting with a single dash
...
public:
  ...
  MainArguments(int argc, char **argv)
      : _programName(argv[0]), _argv(argv + 1, argv + argc) {
    using namespace QuickArgParserInternals;
    if (singleton().initialisationState == UNINITIALISED) {
      // When first created, create temporarily another instance to explore what
      // are the members
      singleton().initialisationState = INITIALISING;

      Child investigator;
      // This will fill the static variables
      singleton().helpPreface
          << QuickArgParserInternals::HelpProvider<Child>::get(
                 [](const std::string &programName) {
                   return programName + " takes between " +
                          std::to_string(singleton().argumentCountMin) +
                          " and " +
                          std::to_string(singleton().argumentCountMax) +
    ...
    if (singleton().initialisationState == INITIALISED) {
      ...
      auto isListedAsString =
          [](const std::string &arg,
             const std::vector<std::pair<std::string, char>> &switches,
             bool unary, bool &skipsNext) {
            for (const auto &it : switches) {
      ...
      auto printHelp = [this]() {
        ...
        QuickArgParserInternals::OnHelpCallback<Child>::on(
            static_cast<Child *>(this), [] { std::exit(0); });
      };
      auto printVersion = [this]() {
        if (!QuickArgParserInternals::VersionPrinter<Child>::print())
          return false; // Returns false if the version is not known, leading to
                        // no action if found

        QuickArgParserInternals::OnVersionCallback<Child>::on(
            static_cast<Child *>(this), [] { std::exit(0); });
        return true;
      };

      // Collect program arguments (as opposed to switches) and validate
      // everything
      for (int i = 0; i < int(_argv.size()); i++) {
        if (switchesEnabled) {
          ...
          if (isListedAsString(_argv[i], singleton().unarySwitches, true,
                               skipsNext)) {
            if (skipsNext)
          ...
          if (_argv[i][0] == '-') {
            ...
            for (int j = 1; j < int(_argv[i].size()); j++) {
              ...
              if (!isListedAsChar(_argv[i][j], singleton().nullarySwitches)) {
                throw ArgumentError(std::string("Unknown switch ") +
                                    _argv[i][j]);
              }
            }
            goto nextArg;
      ...
      if (int(arguments.size()) < singleton().argumentCountMin)
        throw ArgumentError("Expected at least " +
                            std::to_string(singleton().argumentCountMin) +
                            " arguments, got " +
                            std::to_string(arguments.size()));
      if (int(arguments.size()) > singleton().argumentCountMax)
        throw ArgumentError(
            "Expected at most " + std::to_string(singleton().argumentCountMax) +
...
private:
  std::vector<std::string> findOption(const std::string &argument,
                                      char shortcut) const {
    // This returns hogwash if the option is bool, but in that case, we only
    // care that the vector is not empty
    ...
    auto matches = [&](const std::string &matched, int argument) {
      ...
      return matched.size() == _argv[argument].size() ||
             _argv[argument][matched.size()] == '=';
    };
    ...
    for (int i = 0; i < int(_argv.size()); i++) {
          ...
          for (int j = 1; _argv[i][j] != '\0'; j++) {
            if (_argv[i][j] == shortcut) {
              if (_argv[i][j + 1] == '\0') // Last letter, argument follows
                collected.push_back(
                    _argv[std::min<int>(i + 1, _argv.size() - 1)]);
      ...
      if (!argument.empty()) {
        if (matches(argument, i)) {
          if (_argv[i].size() > argument.size() &&
              _argv[i][argument.size()] == '=')
...
protected:
  template <typename Validator> class GrabberBase {
  protected:
    ...
    Validator validator;
    GrabberBase(const MainArguments *parent, const std::string &name,
                char shortcut, const std::string &help,
  ...
  public:
    operator bool() const {
      if (parent->singleton().initialisationState == INITIALISING) {
        parent->singleton().nullarySwitches.push_back(
            std::make_pair(name, shortcut));
        addHelpEntry();
        return false;
      }
...
#if _MSC_VER && !__INTEL_COMPILER
    template <typename T,
              typename std::enable_if<QuickArgParserInternals::StringFilterOk<
    ...
    T getOption(T defaultValue) const {
      if (parent->singleton().initialisationState == INITIALISING) {
        parent->singleton().unarySwitches.push_back(
            std::make_pair(name, shortcut));
        addHelpEntry();
        return defaultValue;
      }
      ...
      auto validate = [&](const T &value) {
        if (!QuickArgParserInternals::ValidatorUser<Validator>::useValidator(
                validator, value)) {
          throw QuickArgParserInternals::ArgumentError(
      ...
      if (!found.empty()) {
        auto obtained =
            QuickArgParserInternals::Demultiplexer<T>::deserialise(found);
        validate(obtained);
        return obtained;
      }
  ...
  public:
    GrabberDefaulted(const MainArguments *parent, const std::string &name,
                     char shortcut, const std::string &help,
                     Validator validator, Default defaultValue)
        : Base(parent, name, shortcut, help, validator),
          defaultValue(defaultValue) {}
#if _MSC_VER && !__INTEL_COMPILER
    template <typename T,
              typename std::enable_if<
                  QuickArgParserInternals::StringFilterOk<T>::value &&
...
#endif
    operator T() const {
      static_assert(QuickArgParserInternals::ArgConverter<T>::canDo,
                    "Cannot deserialise into this type");
      return Base::template getOption<T>(defaultValue);
    }
  };

  template <typename Validator> class Grabber : public GrabberBase<Validator> {
  ...
  public:
    ...
    GrabberDefaulted<Default, Validator> operator=(Default defaultValue) {
      return {Base::parent, Base::name,      Base::shortcut,
              Base::help,   Base::validator, defaultValue};
    }
    ...
    operator T() const {
      static_assert(QuickArgParserInternals::ArgConverter<T>::canDo,
                    "Cannot deserialise into this type");
      return Base::template getOption<T>(
          QuickArgParserInternals::ArgConverter<T>::makeDefault());
    }

    template <typename NewValidator>
    Grabber<NewValidator> validator(const NewValidator &newValidator) {
      return {Base::parent, Base::name, Base::shortcut, Base::help,
              newValidator};
    }
  };

  Grabber<DummyValidator> option(const std::string &name, char shortcut = '\0',
                                 const std::string &help = "") {
    return Grabber<DummyValidator>(this, "--" + name, shortcut, help,
                                   DummyValidator{});
  }
  Grabber<DummyValidator> option(char shortcut = '\0',
                                 const std::string &help = "") {
    return Grabber<DummyValidator>(this, "", shortcut, help, DummyValidator{});
  }
  Grabber<DummyValidator> nonstandardOption(const std::string &name,
                                            char shortcut = '\0',
                                            const std::string &help = "") {
    if (singleton().initialisationState == INITIALISING && name[0] == '-' &&
        name[1] != '-')
  ...
  template <typename Validator> class ArgGrabberBase {
  protected:
    const MainArguments *parent;
    const int index;
    Validator validator;
    template <typename Value> void validate(const Value &value) const {
      if (!QuickArgParserInternals::ValidatorUser<Validator>::useValidator(
              validator, value)) {
        throw QuickArgParserInternals::ArgumentError(
            "Invalid value of argument " + std::to_string(index));
      }
    }

  public:
    ArgGrabberBase(const MainArguments *parent, int index,
                   const Validator &validator)
        : parent(parent), index(index), validator(validator) {}
  };
  ...
  public:
    ArgGrabberDefaulted(const MainArguments *parent, int index,
                        const Validator &validator, Default defaultValue)
    ...
    operator T() const {
      ...
      if (Base::parent->singleton().initialisationState == INITIALISING) {
        Base::parent->singleton().argumentCountMax = std::max(
            Base::parent->singleton().argumentCountMax, Base::index + 1);
  ...
  struct ArgGrabber : public ArgGrabberBase<Validator> {
    ...
    template <typename Default>
    ArgGrabberDefaulted<Default, Validator>
    operator=(Default defaultValue) const {
      return ArgGrabberDefaulted<Default, Validator>{
          Base::parent, Base::index, Base::validator, defaultValue};
    }
    ...
    operator T() const {
      static_assert(QuickArgParserInternals::ArgConverter<T>::canDo,
                    "Cannot deserialise into this type");
      if (Base::parent->singleton().initialisationState == INITIALISING) {
        Base::parent->singleton().argumentCountMin = std::max(
            Base::parent->singleton().argumentCountMin, Base::index + 1);
        Base::parent->singleton().argumentCountMax = std::max(
            Base::parent->singleton().argumentCountMax, Base::index + 1);
        return QuickArgParserInternals::ArgConverter<T>::makeDefault();
      }
